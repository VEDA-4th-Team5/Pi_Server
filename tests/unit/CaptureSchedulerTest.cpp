...
int main() {
    try {
        ...
        //    30s one is not due before T0+30s.
        {
            ...
            require(scheduler.due(t0 + std::chrono::seconds(29)).empty(),
                    "nothing should be due before T0+30s");

            auto due30 = scheduler.due(t0 + std::chrono::seconds(30));
            ...
            require(scheduler.due(t0 + std::chrono::seconds(30)).empty(),
            ...
            require(scheduler.onDispatchResult(
                        r30, true, t0 + std::chrono::seconds(31)) ==
                        DispatchOutcome::Done,
                    "accepted 30s should be Done");

            auto due60 = scheduler.due(t0 + std::chrono::seconds(60));
            ...
            require(scheduler.onDispatchResult(
                        due60.front(), true, t0 + std::chrono::seconds(61)) ==
                        DispatchOutcome::Done,
            ...
            require(scheduler.due(t0 + std::chrono::seconds(120)).empty(),
        ...
        {
            parking::CaptureScheduler scheduler(fastConfig(), ev01Resolver());
            scheduler.onTransition(makeStarted("S2", "EV01", "HALL01", t0));
            const auto again = scheduler.onTransition(
                makeStarted("S2", "EV01", "HALL01", t0));
        ...
        {
            parking::CaptureScheduler scheduler(fastConfig(), ev01Resolver());
            scheduler.onTransition(makeStarted("S3", "EV01", "HALL01", t0));

            auto a1 = scheduler.due(t0 + std::chrono::seconds(30));
            require(a1.size() == 1 && a1.front().attempt == 1, "attempt 1 due");
            // No response within 3s -> retry.
            require(scheduler.onDispatchResult(
                        a1.front(), false, t0 + std::chrono::seconds(33)) ==
                        DispatchOutcome::WillRetry,
                    "first failure should retry");

            // Retry is scheduled at now(33s)+2s = 35s, not before.
            require(scheduler.due(t0 + std::chrono::seconds(34)).empty(),
                    "retry must wait retryInterval");
            auto a2 = scheduler.due(t0 + std::chrono::seconds(35));
            require(a2.size() == 1 && a2.front().attempt == 2, "attempt 2 due");
            require(scheduler.onDispatchResult(
                        a2.front(), false, t0 + std::chrono::seconds(38)) ==
                        DispatchOutcome::WillRetry,
                    "second failure should retry");

            auto a3 = scheduler.due(t0 + std::chrono::seconds(40));
            require(a3.size() == 1 && a3.front().attempt == 3, "attempt 3 due");
            require(scheduler.onDispatchResult(
                        a3.front(), false, t0 + std::chrono::seconds(43)) ==
            ...
            require(scheduler.due(t0 + std::chrono::seconds(50)).empty(),
        ...
        {
            parking::CaptureScheduler scheduler(fastConfig(), ev01Resolver());
            scheduler.onTransition(makeStarted("S4", "EV01", "HALL01", t0));
            scheduler.onTransition(makeCompleted("S4", "EV01"));
            require(scheduler.trackedSessions() == 0,
                    "completed session should be cancelled");
            require(scheduler.due(t0 + std::chrono::seconds(60)).empty(),
