...
- 사용자의 명시적 승인 없이 소스 코드, 설정 파일, 데이터베이스, 문서를 수정하지 않는다.
- 수정 요청을 받더라도 먼저 현재 상태를 읽기 전용으로 조사하고, 근거와 수정 방안을 보고한 뒤 승인을 기다린다.
- 사용자가 명시적으로 구현 또는 수정을 승인한 경우에만 파일을 변경한다.
- 조회, 로그 확인, 빌드 및 테스트처럼 상태를 변경하지 않는 검증은 요청 범위 안에서 수행할 수 있다.
- Python은 절대 사용하지 않는다.
