...
| I-04 Pi ↔ Camera Snapshot/현재 프레임 | `RtspStreamReceiver`, `SnapshotStorage`, `CaptureScheduler`, `HallCaptureExecutor`; BestShot HTTPS 다운로드 | 부분 구현: T0+30/60초 RTSP ROI 저장·OCR 연결 완료, MQTT는 draft 요청, 카메라 응답/요청형 Snapshot API는 미구현 |
...
→ MQTT draft 요청 + RTSP 최신 ROI Snapshot
