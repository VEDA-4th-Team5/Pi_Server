...
본 보고서의 진단에 따라 조치를 2단계로 적용하고 각각 재측정했다.

| 지표 | 조치 전 | 1차 (§8) | **최종 (§9)** | 총 변화 |
|---|---|---|---|---|
| **프로세스 CPU** | **39.59%** | 5.09% | **3.42%** | **11.6배 감소** |
| read 대역폭 | 131.7 MB/s | 0.000 | 0.000 | 계측 하한 이하 |
| read syscall | 33,798 /초 | 17.8 /초 | **4.9 /초** | **6,900배 감소** |
| RSS | 124.3 MB | 83.1 MB | **82.0 MB** | 34% 감소 |
| `database is locked` 누적 77건의 원인 | `busy_timeout` 미적용 | **원인 제거** | — | 적용 확인, 재발 여부는 관찰 필요 |

§7 권고 8개 중 **6개 적용**, 2개는 실익·위험 근거를 들어 보류했다(§9.7).
...
| 1-2 `cache_size` | ✅ 적용 | §8.2, §9.1에서 80MB로 상향 |
| 2-1 INBOX 보존정책 | ✅ **적용 + 실증** | §9.1, §9.4 |
| 2-2 WAL 실적용 검증 | ✅ 적용 | 검증 로그 추가 |
| 3-1 statement 캐시 | ✅ **적용 + 실증** | §9.1, §9.3 |
| 3-2 무의미한 `BEGIN IMMEDIATE` 생략 | ✅ 적용 | §9.1 |
| 3-3 폴링 → 이벤트 구조 전환 | ⏸ **보류** | 실익 1%p 미만, 위험 높음 (§9.5) |
| 3-4 TLS 세션 재사용 | ⏸ **보류** | 절대 비용 1.13%, 클라이언트 측 이슈 (§9.5) |
| 4 조치 후 검증 | ⚠️ 부분 | 60초 측정으로 대체 (원안은 1시간) |

**8개 개선안 중 6개 적용, 2개 보류.** CPU 39.59% → 3.42%(11.6배 감소)를 달성했다.

> 아래 각 절은 최초 작성 시점의 제안 내용이다. 실제 적용 형태와 측정 결과는 §8·§9에 있으며, 일부 제안은 적용 과정에서 형태가 바뀌었다(특히 1-1/1-2의 위치, §8.1 참고).
...
- 측정: 2026-08-27 16:38:07 ~ 16:39:07 (KST), 60초, 재시작 후 30초 워밍업
- 대상: PID 1188807

§8은 §7 우선순위 1과 2-2만 적용한 상태였다. 나머지 중 **2-1, 3-1, 3-2를 추가 적용**했다. 3-3과 3-4는 §9.5의 근거로 보류했다.

### 9.1 적용 내용

| 항목 | 변경 |
|---|---|
| **2-1** INBOX 보존정책 | `purgeSettledSlotTransitionCommands()` + 액터 주기 실행. `OCCUPANCY_INBOX_RETENTION_DAYS=30` |
| **3-1** statement 캐시 | `EventDatabase`에 연결 단위 prepared statement 캐시. 액터 핫패스 4개 쿼리 전환 |
| **3-2** 무의미한 트랜잭션 | `admitDueSlotDeadlines`에 읽기 전용 probe 선행. 마감이 없으면 `BEGIN IMMEDIATE` 생략 |
| `cache_size` | 40MB → 80MB (보존 30일 정상상태 DB 약 70MB 대비) |

**3-1 설계**: 기존 `Statement` RAII에 비소유 생성자를 추가해 캐시된 statement를 빌려 쓴다. 바인딩 메서드를 그대로 재사용하므로 호출부는 한 줄씩만 바뀐다. 수명은 `close()`/`open()` 양쪽에서 `clearStatementCacheUnlocked()`로 관리한다 — `sqlite3_close` 전에 `sqlite3_finalize`가 반드시 선행해야 한다.

**2-1 설계 판단**: 배치 상한(2,000행)을 둔다. `db_mutex_`는 액터 핫패스와 공유되므로 13만 행에서 무제한 `DELETE`를 걸면 액터가 멈춘다. 유입이 시간당 약 184행이라 배치 2,000이면 충분히 따라간다. 또한 정리는 액터의 `progressed`로 치지 않는다 — 세우면 폴링 대기를 건너뛰어 루프가 바쁘게 돈다.

### 9.2 측정 결과 — 전체 조치 누적

| 지표 | 조치 전 (1시간) | 1차 (§8) | **2차 (§9)** | 총 개선 |
|---|---|---|---|---|
| **프로세스 CPU** | **39.59%** | 5.09% | **3.42%** | **11.6배 감소** |
| ┗ user | 21.02% | 3.65% | **2.00%** | 10.5배 감소 |
| ┗ system | 18.57% | 1.45% | **1.42%** | 13.1배 감소 |
| **read syscall** | **33,798/초** | 17.8/초 | **4.9/초** | **6,900배 감소** |
| read 대역폭 | 131.7 MB/s | 0.000 | 0.000 | 계측 하한 이하 |
| RSS | 124.3 MB | 83.1 MB | **82.0 MB** | 34% 감소 |
...
```mermaid
xychart-beta
    title "조치 단계별 프로세스 CPU"
    x-axis ["조치 전", "1차 (PRAGMA)", "2차 (캐시/보존)"]
    y-axis "CPU %" 0 --> 45
    bar [39.59, 5.09, 3.42]
```

60초 구간 편차는 min 3.00% / max 4.20%다.

**교차검증**: perf 549 샘플 ÷ (299 Hz × 60 s) = **3.06%** vs `/proc` **3.42%** — 0.36%p 차이로 일치한다.

### 9.3 3-1 효과 검증

statement 캐시가 실제로 히트하는지는 `sqlite3Parser`로 판별된다. 캐시가 동작하지 않으면 CPU만 그대로이고 코드는 복잡해진 셈이 되므로, 측정 전에 이 판별 기준을 정해두었다.

| 지표 | 1차 (§8) | **2차 (§9)** |
|---|---|---|
| `sqlite3Parser` 자기실행시간 | 3.26% (**flat 1위**) | **0.32%** |
| `Statement::Statement` 누적 | **51.90%** | 프로파일에서 소멸 |
| `libsqlite3` DSO 점유 | 62.29% | **16.36%** |

**재파싱이 제거됐다.**

### 9.4 2-1 검증 — 삭제 대상의 정확성

보존정책은 성능 측정으로는 검증되지 않는다. 30일 조건이라 현재 삭제 대상이 0건이고, 정리 주기가 1시간이라 60초 측정 중에는 호출조차 되지 않는다. **"정상 동작"과 "조용히 아무것도 하지 않음"이 구분되지 않는다.**

라이브 DB의 `.backup` 사본에 경계 조건 4종을 심어 별도 실증했다.

| 심은 행 | 상태 | 기대 | 결과 |
|---|---|---|---|
| `T-old-settled` | 오래됨 + `APPLIED`/`NONE` | 삭제 | ✅ gone |
| `T-old-applied` | 오래됨 + `APPLIED`/`APPLIED` | 삭제 | ✅ gone |
| `T-old-pending` | 오래됨 + `APPLIED`/**`PENDING`** | **보존** | ✅ kept |
| `T-old-unprep` | 오래됨 + **`PENDING_UNPREPARED`** | **보존** | ✅ kept |

```
실데이터 9,292행 전부 잔존 (컷오프 이전 행 없음)
MAX(admission_ordinal) 900004 -> 900004   채번 연속성 유지
PRAGMA integrity_check                     ok
```

**진행 중인 행은 아무리 오래되어도 삭제되지 않음**이 확인됐다.

삭제 안전성의 근거는 세 가지다.

1. **재생(replay) 방어 유지** — 홀 센서 중복 수용을 막는 것은 INBOX 행이 아니라 `OCCUPANCY_SENSOR_SEQUENCE_STATE`와 `evaluateSensorSequence()`다(`EventDatabaseOccupancy.cpp:313`). 이 상태는 삭제 대상이 아니다.
2. **채번 안전** — `MAX(admission_ordinal)`로 채번하는데, 보존 기간이 지난 행만 지우므로 최댓값 보유 행은 삭제되지 않는다.
3. **참조 무결성** — `OCCUPANCY_COMMAND_INBOX`를 참조하는 외래키가 없다.

### 9.5 병목의 이동 — 추가 최적화의 실익

DSO 분포가 완전히 뒤바뀌었다.

| DSO | 조치 전 | **2차 후 (상대)** | **절대 CPU** |
|---|---|---|---|
...
**상대 비중만 보면 오해한다.** TLS가 33%로 1위지만 절대 비용은 **1.13% CPU**다. 전체 파이가 12분의 1로 줄어 각 조각의 지분이 커진 것이다.

이 때문에 **3-3(폴링→이벤트 전환)과 3-4(TLS 세션 재사용)는 적용하지 않았다.**

- **3-3**: 액터 전체가 이제 0.75% CPU(누적 21.95%)다. 동시성 모델 변경으로 얻을 절대량이 1%p 미만인 반면, `SlotTransitionActor`는 슬롯별 순서 보장·ingress/deadline 정렬점·drain 프로토콜을 조율하며 2,074줄 통합 테스트가 그 semantics를 검증한다. 위험 대비 실익이 없다.
- **3-4**: 절대 비용 1.13%. cpp-httplib은 이미 keep-alive가 기본 활성(`keep_alive_timeout_sec=5`)이라 코드에 누락된 설정이 없다. 핸드셰이크가 보이는 것은 클라이언트 폴링 간격이 5초를 넘어 연결이 만료되기 때문이다. 타임아웃을 늘리면 소켓과 워커 스레드를 더 오래 점유해 동시 접속 수용량과 맞바꾸게 된다. **클라이언트(Qt) 폴링 주기 조정이 올바른 접근이다.**
...
실패 4건은 **모두 기존 결함으로, 본 변경과 무관함을 실증했다.**

| 테스트 | 실패 사유 |
|---|---|
| `fire-alarm-test` | `no such table: main.PARKING_SESSION` |
| `fire-delivery-integration-test` | 〃 |
| `auth-service-test` | 〃 |
| `fire-broker-restart-process-test` | `mosquitto exited during startup` (브로커 부재, 환경) |

원인은 `EventDatabaseTimer.cpp:692`가 가드 없이 `CREATE UNIQUE INDEX ... ON PARKING_SESSION`을 실행하는 것이다. `PARKING_SESSION`은 `db/schema.sql`에서만 생성되며 이는 `initialize()`가 실행하는데, 해당 테스트들은 `migrateRuntimeSchema()`만 호출한다. 이 줄은 본 브랜치의 부모 커밋 `08a0341`(2026-08-24, EVDA-238)에서 추가됐다.

두 가지로 실증했다.

```
# (1) 변경을 stash한 상태로 빌드해 동일 실패 재현 (빌드 에러 0건 확인)
fire-alarm-test    FAIL -> no such table: main.PARKING_SESSION
auth-service-test  FAIL -> no such table: main.PARKING_SESSION

# (2) PRAGMA 유무와 무관함을 SQLite CLI로 직접 확인
$ sqlite3 fresh.db "CREATE UNIQUE INDEX IF NOT EXISTS ... ON PARKING_SESSION(...)"
Error: no such table: main.PARKING_SESSION
$ sqlite3 fresh2.db "PRAGMA journal_mode=WAL; PRAGMA optimize; PRAGMA cache_size=-80000; CREATE UNIQUE INDEX ..."
Error: no such table: main.PARKING_SESSION
```

**본 변경으로 인한 회귀는 0건이다.** 액터와 DB 경로를 함께 검증하는 `slot-transition-actor-test`(2,074줄)는 통과했다.

> 이 기존 결함은 본 작업 범위 밖이라 수정하지 않았다. 별도 처리가 필요하다.

### 9.7 최종 반영 현황

| 항목 | 상태 |
|---|---|
| 1-1 `PRAGMA optimize` | ✅ 적용 (§8) |
| 1-2 `cache_size` | ✅ 적용 (§8, §9에서 80MB로 상향) |
| 2-1 INBOX 보존정책 | ✅ **적용 + 실증** (§9.4) |
| 2-2 WAL 실적용 검증 | ✅ 적용 (§8) |
| 3-1 statement 캐시 | ✅ **적용 + 실증** (§9.3) |
| 3-2 `BEGIN IMMEDIATE` 생략 | ✅ 적용 |
| 3-3 폴링 → 이벤트 전환 | ⏸ 보류 — 실익 1%p 미만, 위험 높음 (§9.5) |
| 3-4 TLS 세션 재사용 | ⏸ 보류 — 절대 비용 1.13%, 클라이언트 측 이슈 (§9.5) |

**8개 중 6개 적용.** 보류 2개는 근거를 명시했다.
